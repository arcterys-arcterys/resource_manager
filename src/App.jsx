...
export default function App() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const confirmedData = cRes.data;
      const confirmedResIdSet = new Set(confirmedData.map(c => c.reservation_id));
      const confirmedEquipIdSet = new Set(confirmedData.map(c => c.equip_id));
      // Only show reservations/equipment not yet confirmed
      setQueue(qRes.data
        .filter(r => !confirmedResIdSet.has(r.id))
        .map(r => ({ ...r, prefs: r.prefs ?? [] })));
      setEquipment(eRes.data.filter(e => !confirmedEquipIdSet.has(e.id)));
  ...
  const removeFromQueue = (id) => withBusy(async () => {
    await supabase.from('denied_pairs').delete().eq('reservation_id', id);
  ...
  const removeEquipment = (id) => withBusy(async () => {
    // Only delete if not confirmed — confirmed equipment stays as a record
    const isConfirmed = confirmed.some(c => c.equip_id === id);
  ...
  const confirmMatch = ({ reservation, equip }) => withBusy(async () => {
    const { error } = await supabase.from('confirmed_matches').insert({
