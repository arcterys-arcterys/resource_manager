...
export default function App() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      setEquipment(eRes.data.map(e => ({ ...e, desc: e.description ?? e.desc ?? '' })));
      setConfirmed(cRes.data.map(c => ({ ...c, equip_desc: c.equip_description ?? c.equip_desc ?? '' })));
      setDenied(dRes.data);
      setSyncStatus("ok");
      setGlobalError(null);
    } catch (err) {
  ...
  // ── Match actions ──
  // FIX: use atomic DB function that checks both sides aren't already confirmed
  // before inserting — prevents race conditions when two users confirm simultaneously
  const confirmMatch = ({ reservation, equip }) => withBusy(async () => {
    const { data, error } = await supabase.rpc('confirm_match', {
      p_reservation_id: reservation.id,
      p_reservation_name: reservation.name,
      p_equip_id: equip.id,
      p_assigned_type: equip.letters,
      p_assigned_number: equip.number,
      p_equip_description: equip.desc,
    });
    if (error) throw new Error(error.message);
  ...
  const denyMatch = ({ reservation, equip }) => withBusy(async () => {
    // unique constraint on (reservation_id, equip_id) handles duplicate denials gracefully
  ...
  // FIX: confirm all pending matches sequentially using the atomic function
  // so each one independently checks for conflicts rather than bulk-inserting
  const confirmAll = () => withBusy(async () => {
    for (const { reservation, equip } of pending) {
      const { data, error } = await supabase.rpc('confirm_match', {
        p_reservation_id: reservation.id,
        p_reservation_name: reservation.name,
        p_equip_id: equip.id,
      ...
      // Skip already-confirmed pairs silently; throw on real errors
      if (error) throw new Error(error.message);
    }
  });
                ...
                <tbody>
                  {pending.map(({ reservation, equip }, i) => (
                    <tr key={reservation.id}>
                      ...
                      <td className="col-desc">{equip.desc || <span style={{ color: "#2a2a2a" }}>—</span>}</td>
                      <td>
        ...
        <div className="main-grid">
          ...
          <div className="panel">
            <div className="panel-header">
              ...
              <span className="panel-count">{queue.length} total</span>
            </div>
            <div className="panel-body">
              ...
              <div className="list">
                {queue.length === 0 && <div className="empty">Queue is empty.</div>}
                {queue.map((r, i) => {
                  const isPending = pendingResIds.has(r.id);
                  const isConfirmed = confirmedResIds.has(r.id);
                  return (
                      ...
                      <div className="list-item-left">
                        ...
                        <div style={{ minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center" }}>
                            ...
                            {isConfirmed && <span className="tag tag-green" style={{ marginLeft: 8 }}>✓</span>}
                          </div>
                          {r.prefs.length > 0 &&
                    ...
                    <div className={`list-item${isPending ? " is-pending" : ""}`} key={e.id}>
                      <div className="list-item-left">
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          {e.desc && <div className="item-sub">{e.desc}</div>}
                        </div>
                      </div>
