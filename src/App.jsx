...
export default function App() {
  ...
  const confirmMatch = ({ reservation, equip }) => withBusy(async () => {
    ...
    if (error) throw new Error(error.message);
    // Remove from source pools after confirming
    await supabase.from('reservations').delete().eq('id', reservation.id);
    await supabase.from('equipment').delete().eq('id', equip.id);
  });
  ...
  const confirmAll = () => withBusy(async () => {
    ...
    if (error) throw new Error(error.message);
    // Remove all confirmed items from source pools
    await supabase.from('reservations').delete().in('id', toConfirm.map(p => p.reservation.id));
    await supabase.from('equipment').delete().in('id', toConfirm.map(p => p.equip.id));
  });
